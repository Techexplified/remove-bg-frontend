...
async function pushPreview() {
    const sel = figma.currentPage.selection.filter(isExportable);
    if (sel.length === 0) {
        figma.ui.postMessage({ type: "preview-update", payload: null });
        return;
    }
    try {
...
figma.ui.onmessage = async (msg) => {
    switch (msg.type) {
        ...
        case "export-selected-image": {
            const sel = figma.currentPage.selection.filter(isExportable);
            if (sel.length !== 1) {
                figma.ui.postMessage({ type: "export-error", payload: "Select a single layer first." });
                break;
            }
            try {
                ...
                figma.ui.postMessage({ type: "export-error", payload: e instanceof Error ? e.message : "Export failed." });
            }
            break;
        }
        case "insert-result-image": {
            ...
            try {
                ...
                figma.notify(e instanceof Error ? e.message : "Couldn't place result.");
            }
            break;
        }
        case "resize-selection": {
            const { width, height } = msg;
            const sel = figma.currentPage.selection.filter((n) => "resize" in n);
            if (sel.length === 0) {
                figma.notify("Select a layer to resize.");
                break;
            }
        ...
        case "apply-image-adjustments": {
            const { brightness, contrast, saturation, opacity, backgroundPreset, imageFilterPreset } = msg;
            ...
            for (const node of sel) {
                ...
                if (imageFilterPreset && imageFilterPreset !== "normal") {
                    switch (imageFilterPreset) {
                        case "grayscale":
                            filterOverrides.saturation = -1;
                            break;
                        case "sepia":
                            filterOverrides.saturation = -0.5;
                            filterOverrides.temperature = 0.4;
                            filterOverrides.tint = 0.1;
                            break;
                        case "vivid":
                            filterOverrides.saturation = 0.6;
                            filterOverrides.contrast = 0.15;
                            filterOverrides.highlights = 0.1;
                            break;
                        case "cool":
                            filterOverrides.temperature = -0.5;
                            filterOverrides.shadows = 0.1;
                            break;
                        case "warm":
                            filterOverrides.temperature = 0.35;
                            filterOverrides.tint = 0.1;
                            break;
                        case "fade":
                            filterOverrides.saturation = -0.2;
                            filterOverrides.contrast = -0.2;
                            filterOverrides.exposure = 0.1;
                            filterOverrides.highlights = 0.2;
                            break;
                        case "matte":
                            filterOverrides.saturation = -0.2;
                            filterOverrides.contrast = -0.15;
                            filterOverrides.exposure = 0.05;
                            filterOverrides.shadows = 0.15;
                            break;
                        case "dramatic":
                            filterOverrides.contrast = 0.4;
                            filterOverrides.saturation = 0.2;
                            filterOverrides.exposure = -0.1;
                            break;
                        case "retro":
                            filterOverrides.saturation = -0.3;
                            filterOverrides.temperature = 0.2;
                            filterOverrides.tint = 0.05;
                            filterOverrides.contrast = 0.1;
                            break;
                    }
                }
                const updatedImgFill = Object.assign(Object.assign({}, imgFill), { filters: Object.assign(Object.assign({}, baseFilters), filterOverrides) });
                if (backgroundPreset) {
        ...
        case "export-node-by-id": {
            ...
            if (!node || !isExportable(node)) {
                figma.ui.postMessage({ type: "export-node-result", payload: { requestId, error: "Layer not found." } });
                break;
            }
            try {
