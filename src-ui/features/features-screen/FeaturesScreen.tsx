...
export function FeaturesScreen({ onRunFeature, onManagePlan, onTopUp, watching, onStopWatching }: Props) {
  ...
  const primaryDisabled = isBusy || !status || spendable < 1 || (imageSource === "canvas" && (multiSel || !hasSel));
  ...
  return (
    <>
      <div className="pane-header">
        <h2>RemoveBG</h2>
        <p>AI-powered background tools</p>
      </div>
  ...
  return (
    <div className="feature-group">
      ...
      {features.map(f => {
        ...
        const disabled = (!isComingSoon && unlocked && (isBusy || spendable < f.credits)) || isComingSoon;
