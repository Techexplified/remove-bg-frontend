...
import { reactivateSubscription, ApiError } from "../../shared/api/client";
  ...
  return (
    <div className="hero">
      ...
      {isCancelling && status.isActive && (
        <div className="hero-notice hero-notice-cancel" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
          <div>
            <strong>⚠ Your subscription cancels on {fmtDate(status.scheduledCancelAt)}.</strong>
            <div style={{ opacity: 0.85, fontSize: "9px", marginTop: "2px" }}>You keep full access until then.</div>
          </div>
          <button
          ...
          >
            <RefreshCw size={10} style={{ animation: reactivating ? "spin 0.8s linear infinite" : undefined }} />
